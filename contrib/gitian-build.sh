...
VERSION=
commit=false
url=https://github.com/liray-unendlich/elecash
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the elecash, gitian-builder, gitian.sigs, and elecash-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/liray-unendlich/elecash
...
then
	# Make output folder
	mkdir -p "./elecash-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit elecash=${COMMIT} --url elecash=${url} ../elecash/contrib/gitian-descriptors/gitian-linux.yml
	    ...
	    mv build/out/elecash-*.tar.gz build/out/src/elecash-*.tar.gz ../elecash-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit elecash=${COMMIT} --url elecash=${url} ../elecash/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/elecash-*.zip build/out/elecash-*.exe ../elecash-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit elecash=${COMMIT} --url elecash=${url} ../elecash/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/elecash-*.tar.gz build/out/elecash-*.dmg ../elecash-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit elecash=${COMMIT} --url elecash=${url} ../elecash/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../elecash/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/elecash-*.tar.gz build/out/src/elecash-*.tar.gz ../elecash-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../elecash/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/elecash-*win64-setup.exe ../elecash-binaries/${VERSION}
	    mv build/out/elecash-*win32-setup.exe ../elecash-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../elecash/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/elecash-osx-signed.dmg ../elecash-binaries/${VERSION}/elecash-${VERSION}-osx.dmg
	fi
	popd || exit
