...
CzELCWallet::CzELCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zelc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZELCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZELCSeed_deprecated()) {
                    LogPrintf("%s: Updated zELC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zelc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zelc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzELCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZELCCount(nCountLastUsed);
    else if (!walletdb.ReadZELCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzELCWallet::Lock()
{
    seedMaster = 0;
}

void CzELCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzELCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZELC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzELCWallet::LoadMintPoolFromDB()
{
...
void CzELCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzELCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzELCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zelcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzELCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZELC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zelcTracker which also adds to database
    pwalletMain->zelcTracker->Add(dMint, true);
...
void CzELCWallet::SeedToZELC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzELCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzELCWallet::GenerateDeterministicZELC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzELCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzELCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
