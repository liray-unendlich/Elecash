...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered elecash address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ELC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal elecash:URI (elecash:XyZ...?message=example)
            {
...
bool WalletModel::setWalletEncrypted(bool encrypted, const SecureString& passelcase)
{
    if (encrypted) {
        // Encrypt
        return wallet->EncryptWallet(passelcase);
    } else {
...
bool WalletModel::changePasselcase(const SecureString& oldPass, const SecureString& newPass)
{
    bool retval;
    {
        LOCK(wallet->cs_wallet);
        wallet->Lock(); // Make sure wallet is locked before attempting pass change
        retval = wallet->ChangeWalletPasselcase(oldPass, newPass);
    }
    return retval;
}
...
{
    setMints.clear();
    setMints = pwalletMain->zelcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
