...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sELCPercentage, QString& szELCPercentage)
{
    int nPrecision = 2;
    ...
    szELCPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sELCPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount elcAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = elcAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // ELC labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, elcAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelELCPercent->setText(sPercentage);
    ui->labelzELCPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zELC.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in elecash.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in elecash.conf");
    }
    ...
    bool showELCAvailable = settingShowAllBalances || elcAvailableBalance != nTotalBalance;
    bool showWatchOnlyELCAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showELCPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyELCPending = watchUnconfBalance != 0;
    bool showELCLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyELCLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showELCAvailable || showWatchOnlyELCAvailable);
    ui->labelBalanceText->setVisible(showELCAvailable || showWatchOnlyELCAvailable);
    ui->labelWatchAvailable->setVisible(showELCAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showELCPending || showWatchOnlyELCPending);
    ui->labelPendingText->setVisible(showELCPending || showWatchOnlyELCPending);
    ui->labelWatchPending->setVisible(showELCPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showELCLocked || showWatchOnlyELCLocked);
    ui->labelLockedBalanceText->setVisible(showELCLocked || showWatchOnlyELCLocked);
    ui->labelWatchLocked->setVisible(showELCLocked && showWatchOnly);
    ...
    bool showzELCAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzELCUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzELCImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzELCAvailable);
    ui->labelzBalanceMatureText->setVisible(showzELCAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzELCUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzELCUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzELCImmature);
    ui->labelzBalanceImmatureText->setVisible(showzELCImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelELCPercent->setVisible(showPercentages);
    ui->labelzELCPercent->setVisible(showPercentages);
