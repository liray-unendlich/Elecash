...
bool isValidPasselcase(QString strPasselcase, QString& strInvalid)
{
    for (int i = 0; i < strPasselcase.size(); i++) {
        if (!validChar.contains(strPasselcase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(strPasselcase[i]))
                continue;

            strInvalid = strPasselcase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qstrPasselcase = ui->passelcaseIn_ENC->text();
    QString strInvalid;
    if (!isValidPasselcase(qstrPasselcase, strInvalid)) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_ENC->setText(tr("The entered passelcase is invalid. ") + strInvalid + QString(" is not valid") + QString(" ") + tr("Allowed: 0-9,a-z,A-Z,") + specialChar);
        return;
    }
    ...
    std::string encryptedKey = BIP38_Encrypt(EncodeDestination(addr), qstrPasselcase.toStdString(), key.GetPrivKey_256(), key.IsCompressed());
...
{
    string strPasselcase = ui->passelcaseIn_DEC->text().toStdString();
    ...
    if (!BIP38_Decrypt(strPasselcase, strKey, privKey, fCompressed)) {
        ...
        ui->statusLabel_DEC->setText(tr("Failed to decrypt.") + QString(" ") + tr("Please check the key and passelcase and try again."));
        return;
    }
