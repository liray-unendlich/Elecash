...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ELC);
    ...
    if (!settings.contains("nAnonymizeElecashAmount"))
        settings.setValue("nAnonymizeElecashAmount", 1000);

    nAnonymizeElecashAmount = settings.value("nAnonymizeElecashAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeElecashAmount"))
        SoftSetArg("-anonymizeelecashamount", settings.value("nAnonymizeElecashAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in elecash.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeElecashAmount:
            return QVariant(nAnonymizeElecashAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeElecashAmount:
            nAnonymizeElecashAmount = value.toInt();
            settings.setValue("nAnonymizeElecashAmount", nAnonymizeElecashAmount);
            emit anonymizeElecashAmountChanged(nAnonymizeElecashAmount);
            break;
        case CoinControlFeatures:
