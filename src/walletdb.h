...
public:
    ...
    bool WriteZELCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZELCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZELCSeed_deprecated(uint256& seed);
    bool EraseZELCSeed();
    bool EraseZELCSeed_deprecated();

    bool WriteZELCCount(const uint32_t& nCount);
    bool ReadZELCCount(uint32_t& nCount);
