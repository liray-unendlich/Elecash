...
#ifndef ELECASH_ZELCWALLET_H
#define ELECASH_ZELCWALLET_H
...
public:
    CzELCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZELC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZELC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ELECASH_ZELCWALLET_H
